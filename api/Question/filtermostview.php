<?php
    ...
    //IF TOPIC ID DOESNT EXIST PROCEED WITH NORMAL DISPLAYING WITH FILTERING VIEWS DESCENDING
    if(!$tID){

        //FETCH ALL QUESTIONS FROM QUESTIONS TABLE BY MOST VIEWS, ALSO CREATE TEMP TABLE FROM ADVICE AND STORE AS COMMENTCOUNT
        $dataquery = mysqli_query($con, "SELECT *, (select count(*) from advice where advice.question_id = questions.Question_ID) AS CommentCount FROM Questions ORDER BY views DESC");
    ...
    //IF TOPIC ID EXIST
        ...
        while($h = mysqli_fetch_object($t_query)){
            ...
            //FETCH ALL QUESTIONS FROM QUESTIONS TABLE WITH SELECTED TOPIC ORDERED BY AMOUNT OF VIEWS, ALSO CREATE TEMP TABLE FROM ADVICE AND STORE AS COMMENTCOUNT
            $dataquery = mysqli_query($con, "SELECT *, (select count(*) from advice where advice.question_id = questions.Question_ID) AS CommentCount FROM Questions WHERE topic='$tName' ORDER BY views DESC");
