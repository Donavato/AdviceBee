...
while($f = mysqli_fetch_object($fquery)){
    ...
    $dataquery = mysqli_query($con, "SELECT *, (select count(*) from advice where advice.question_id = questions.Question_ID) AS CommentCount FROM Questions WHERE user_id='$f_userID' ORDER BY timestamp DESC");
    $arr = array();
