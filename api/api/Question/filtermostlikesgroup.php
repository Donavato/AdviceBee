<?php
    ...
    //GRAB TOPIC ID FROM SESSION
    $tID=$_POST['gID'];

    //IF TOPIC ID DOESNT EXIST PROCEED WITH NORMAL DISPLAYING WITH FILTERING LIKES DESCENDING
    if(!$tID){

        //FETCH ALL QUESTIONS FROM QUESTIONS TABLE BY MOST LIKES, ALSO CREATE TEMP TABLE FROM ADVICE AND STORE AS COMMENTCOUNT
        $dataquery = mysqli_query($con, "SELECT *, (select count(*) from advice where advice.question_id = questions.Question_ID) AS CommentCount FROM Questions where subtopic IS NOT NULL ORDER BY likes DESC");
    ...
    //IF TOPIC ID EXIST 
        ...
        $t_query = mysqli_query($con, "SELECT * FROM `subtopic` WHERE group_id='$tID'");

        //STORE TOPIC NAME INTO VARIABLE WITH RELATING TOPIC ID
        while($h = mysqli_fetch_object($t_query)){
            $tName = $h->group_name;
    
            //FETCH ALL QUESTIONS FROM QUESTIONS TABLE WITH SELECTED TOPIC ORDERED BY AMOUNT OF LIKES, ALSO CREATE TEMP TABLE FROM ADVICE AND STORE AS COMMENTCOUNT
            $dataquery = mysqli_query($con, "SELECT *, (select count(*) from advice where advice.question_id = questions.Question_ID) AS CommentCount FROM Questions WHERE subtopic='$tName' ORDER BY likes DESC");
