<?php
    ...
    if(!$tID){
        ...
        $dataquery = mysqli_query($con, "SELECT *, (select count(*) from advice where advice.question_id = questions.Question_ID) AS CommentCount FROM Questions where subtopic IS NOT NULL ORDER BY timestamp DESC");
        $arr = array();
        ...
        while($h = mysqli_fetch_object($t_query)){
            ...
            $dataquery = mysqli_query($con, "SELECT *, (select count(*) from advice where advice.question_id = questions.Question_ID) AS CommentCount FROM Questions WHERE subtopic='$tName' ORDER BY timestamp DESC");
            $arr = array();
